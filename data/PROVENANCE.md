...
|:---|:---|:---|
| Entry-duration and entry-share plot, `figures/entry-cost.tex` | `h200/numbers/paired_control_share.json` | EP8, T8/32/64/128; pre only |
| Entry constants, Table `tab:entry-constants` | `h200/numbers/pre_min_fits_t64.json` | All listed T enter the estimate; no held-out claim |
| Completion observations, `figures/completion-cost.tex` | `h200/numbers/signal_refit_20260914.json` and `signal_probe_20260914.json` | Main refit plus probes; T32 pools six process-run means |
...
The entry constants are 11.65/11.54 us (EP4), 14.96/14.94 us (EP8), and
...
Completion measurements report 13.97/13.56 us at T8, 13.98/13.78 at T16,
...
Its signal rows are not the source of the current completion plot.
...
The completion window starts after the local flush/join, before signal issuance,
...
The plotted share is not request latency; a narrow completion share was not collected per layer.
