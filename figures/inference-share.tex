...
\begin{axis}[width=\columnwidth,height=5.4cm,
 ...
 xlabel={Request concurrency},ylabel={Entry share of GPU step (\%)},
...
\end{axis}
\end{tikzpicture}
\caption{Qwen3-30B-A3B decode at fixed EP8. Entry intervals for Dispatch and Combine are paired with the same rank's GPU step. Error bars describe three client windows, not server restarts.}
\label{fig:inference-share}
\Description{Grouped bars compare one-node and two-node EP8 decode at request concurrency sixty-four and one hundred twenty-eight. Single-node entry shares are around eight to nine percent, and cross-node shares around fifteen to seventeen percent.}
\end{figure}
